...
function updateClockAndDefaultTime(force=false){
  $('clock').textContent=nowClock();
  $('dateText').textContent=fullDate();
  const t=nowTime();
  if(force || (autoUrineTime && t!==lastAutoMinute)){
    $('urineTime').value=t;
    lastAutoMinute=t;
  }
}
function defaults(){updateClockAndDefaultTime(true)}
...
function saveUrine(){addOrUpdate('urine',{time:$('urineTime').value||nowTime(),ml:urineMl(),before:$('beforeKg').value,after:$('afterKg').value,memo:$('urineMemo').value});$('beforeKg').value='';$('afterKg').value='';$('urineMl').value='';$('urineMemo').value='';autoUrineTime=true;updateClockAndDefaultTime(true)}
...
function clearToday(){if(confirm('今日の記録をすべて削除しますか？')){const all=data();delete all[today()];saveData(all);renderAll()}}
function renderAll(){renderQuick();renderRecords()}

$('urineTime').addEventListener('input',()=>{autoUrineTime=false});
$('urineTime').addEventListener('focus',()=>{autoUrineTime=false});
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden){autoUrineTime=true;updateClockAndDefaultTime(true)}});

defaults(); renderAll();
setInterval(updateClockAndDefaultTime,1000);
